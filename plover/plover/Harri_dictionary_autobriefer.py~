...
folding_rules = {
    ...
    #Josiah
    'vowel/ → ^-'       : True,
    ## check this with ready
    '/y → *D'           : True,

    #Harri
    'ex/ → ^SK-'        : True,
    'v  → -FB'          : True,    #Josiah uses this for conflicts
    'z → STKPW-'        : False,
    '/chure → -FRP'     : False,
    '/ckchure → *FRP'   : False,
...
list_of_dictionaries = [ "lapwing-base",
                         "lapwing-proper-nouns",
                         "lapwing-uk-additions"
    ...
    #with (open(".config/plover/"+ dictionary_file, "r", encoding="utf-8")) as temp_dictionary:                      #Linux
        ...
        for outline in temp_dictionary:
            ...
            while unchecked_outlines_to_add:
                ...
                if folding_rules['*R -> re-']:
                    # matches with those that don't have R or *
                    # 2014 matches
                    match = re.fullmatch(r'(.*[/X]#?\+?\^?)RAOE/(S?T?K?P?W?H?R?A?O?)\-?(E?U?F?)(P?B?L?G?T?S?D?Z?[/X].*)', working_outline)
                    if match:
                        unchecked_outlines_to_add.append(match[1] +  match[2] + "*" + match[3] + "R" + match[4])

                    # matches with those that don't have * and have R
                    # 2496 - 2014 = 482 matches
                    match = re.fullmatch(r'(.*[/X]#?\+?\^?)RAOE/(S?T?K?P?W?H?R?A?O?)\-?(E?U?F?R)(P?B?L?G?T?S?D?Z?[/X].*)', working_outline)
                    if match:
                        unchecked_outlines_to_add.append(match[1] +  match[2] + "*" + match[3] + match[4])

                    # matches with no R but *
                    # 2496 - 2447 = 49 matches
                    match = re.fullmatch(r'(.*[/X]#?\+?\^?)RAOE/(S?T?K?P?W?H?R?A?O?\*)\-?(E?U?F?)(P?B?L?G?T?S?D?Z?[/X].*)', working_outline)
                    if match:
                        unchecked_outlines_to_add.append(match[1] +  match[2] + match[3] + "R" + match[4])


                if folding_rules['/*ER and /O*R -> -R']:
                    # folding er into words
                    # might also fold or and ar into words

                    # 1373 matches
                    match = re.fullmatch(r'(.*[/X]#?\+?\^?)(S?T?K?P?W?H?R?[AO\-\*EU]+F?)(P?B?L?G?T?S?D?Z?)/\*ER([/X].*)', working_outline)
                    if match:
                        unchecked_outlines_to_add.append(match[1] + match[2] + "R" + match[3] + match[4])


                if folding_rules['BL -> low or lo']:
                    # 303 matches
                    match = re.fullmatch(r'(.*[/X]#?\+?\^?)(S?T?K?P?W?H?R?[AO\-\*EU]+F?R?P?)(G?T?S?D?Z?)/HROE([/X].*)', working_outline)
                    if match:
                        unchecked_outlines_to_add.append(match[1] + match[2] + "BL" + match[3] + match[4])

                if folding_rules['-FRP -> -chr']:
                    # 252 matches
                    # might augment this with churity churly like stuff
                    # currently this only does the chr and no addition endings
                    # would have to check endings to make sure those keys aren't already used
                    match = re.fullmatch(r'(.*[/X]#?\+?\^?)(S?T?K?P?W?H?R?[AO\-\*EU])(B?L?G?T?S?D?Z?)/KHUR([/X].*)', working_outline)
#                    match = re.fullmatch(r'(.*[/X]#?\+?\^?)(S?T?K?P?W?H?R?[AO\-\*EU])(B?L?G?T?S?D?Z?)/KHUR(B?L?G?T?S?D?Z?)([/X].*)', working_outline)
                    # this allos for KHURZ to also be converted but it seems to match with less things which seems weird to be
                    # don't fully trust it tho I don't this it actually does any folding
#                    match = re.fullmatch(r'(.*[/X]#?\+?\^?)(S?T?K?P?W?H?R?[AO\-\*EU]+)/KHUR(B?L?G?T?S?D?Z?[/X].*)', working_outline)
                    if match:
                        unchecked_outlines_to_add.append(match[1] + match[2] + "FRP" + match[3] + match[4])
#                        unchecked_outlines_to_add.append(match[1] + match[2] + "FRP" + match[3] + match[4] + match[5])
